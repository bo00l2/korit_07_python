...
# 함수 정의 영역
def is_resources_enough(order_ingredient): # 만약에 특정 함수 / 메서드의 결과값이 boolean이라면 대개 다음 조건문/반복문의 조건식으로 쓰이는 경우가 많다. 함수형 프로그래밍 개념을 떠올리면 된다.
    """
    DocString : 함수 / 클래스 / 메서드가 어떤 작동을 하는지 '사람들에게' 설명해주는 기능.
    주문 받은 음료를 resources에서 재료 차감을 하고 난 후, 음료 만들기가 가능하면 True 반환.
        아니면 False 반환
...
def process_coins():
    ...
    sum = 0
    # 이 부분에 로직이 들어가야 한다.
    # quarter = float(input('쿼터 동전을 입력하세요 >>> ')) * 0.25
    # dime = float(input('다임 동전을 입력하세요 >>> ')) * 0.1
    # nickel = float(input('니켈 동전을 입력하세요 >>> ')) * 0.05
    # penny = float(input('페니 동전을 입력하세요 >>> ')) * 0.01

    # 근데 지금 함수 하나에 지역 변수가 5 개나 있다는 사실을 알 수 있다.
...
def is_transaction_successful(money_received, drink_cost):
    ...
    if change >= 0:
        ...
        return False    # 얘는 음료 제조 과정의 조건식으로 쓰이더라도 실행 x

def make_coffee(drink_name, order_ingredient):
    """
    recources에 있는 재료에서 메뉴['음료명']['재료]들을 차가맘.
    ...
    """
    for item in order_ingredient:                   # resources가 아니라 order_ingredient를 반복문 돌리는 이유는 에스프레소일 때 오류가 발생하기 때문이다. 에스프레소의 재료에는 우유가 없는데 resources에는 있는 상태가 된다. 그래서 order_ingredient['우유'] 부분에서 오류가 발생하게 된다.
        resources[item] -= order_ingredient[item]
    print(f'{choice}☕이(가) 나왔습니다. 맛있게 드세요 !! ')

#todo - 1 : 커피 머신이 반복적으로 돌아가야하는데, off를 입력 받으면 종료가 이루어져야 한다.

# 관련 변수 선언 및 초기화
is_on = True
while is_on:
    # 반복문 내부에서 입력 받아야 하니까 여기다가 선언 및 초기화
    choice = input('어떤 음료를 드시겠습니까? (에스프레소 / 라떼 / 카푸치노) >>> ')
    # 만약에 choice 변수에 들어간 데이터가 'off'라면 반복문을 종료하도록 나머지 코드를 작성하시오.
    # todo - 2 : 만약에 choice 변수에 들어간 데이터가 'off'라면 반복문 종료하도록 나머지 코드 작서
    if choice == 'off':
        print('자판기가 종료되었습니다. ')
        break
    ...
    #todo - 4 : choice == 에스프레소 / 라떼 / 카푸치노 중 하나일 때 작성하는 부분
        ...
        # 3. T라면 음료를 반환해야하는데, resources에 잇는 수량을 감소시키고, profit에는 음료 가격만큼 증가시킬 필요 있음.
        # 리고 동전 받은 것에서 음료 가격만큼 빼고 반환도 해줘야 함.
        if is_resources_enough(MENU[choice]['재료']):
            # 이상의 조건식이 True라면 동전 처리해야한다.
            ...
            if is_transaction_successful(money_received=money_received, drink_cost=MENU[choice]['가격']):
                # 이제 여기에 음료 제조 과정을 작성하면 되겠다.
                # 재료를 이제 실질적으로 차감하고, 음료를 만들어서 사용자에게 제공해야한다.
                # 재료 차감 파트를 is_resources_enough() 함수를 참조하여 여기서 작성
                # 해당 작성 결과를 함수화할 예정
                # resources[choice] -= MENU[choice]['재료']
                #
