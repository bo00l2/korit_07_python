...
#     result = b / a
'''
근데 우리는 어제 사용해봤었다. coffee_machine에서.
if drink == None:
    이라는 방식으로 사용해왔다.
...
#     print('0으로 나눌 수 없습니다.')
'''
        거의 모든 예외는 Exception 클래스의 서브 클래스에 해당함. 따라서 모든 예외는 Exception의 인스턴스가 된다. 다음과 같은 except의 마지막에 Exception을 명시하면 예상하지 못한 예외들도 웬만하면 처리 된다.
...
3. 예외 메시지 처리하기
    이상까지는 특정 예외가 발생했을 때 저희가 메시지를 커스텀해서 사용했지만 기본적으로 이미 예외 메시지를 가지고 있는 경우도 있다. default exception message 를 출력하는 방식 학습할 거다.
...
'''
4. else / finally
    try / except 문에 else / finally 를 달 수 있는데, 
...
#     print('프로그램이 종료되었습니다.')
'''
ZeroDivisionError를 예외 처리했음에도 불구하고
a = 0
b = 아무 정수
를 넣었을 때 ZeroDivisionError가 예외 처리되지 않고 오류 발생을 하는 것을 확인할 수 있다.
어떻게 처리할지 고민해보기
'''

'''
5. 강제로 예외를 발생시키기
    어떤 사람이 나이를 정수로 입력받은 프로그램을 사용한다고 가정했을 때,
    컴퓨터 상으로는(그리고 파이썬 상으로는) -1000이 정수이기 때문에 예외가 발생하지 않는다.(그래서 이전에 0미만 200초과를 조건문으로 처리하는 것을 학습했었다)
    하지만 -1000살이 되는 것이 불가능하기 때문에 조건문이 아니라 직접 예외를 발생시켜서 처리하는 방법을 학습한다. -> raise 문
...
'''
# age = int(input('나이를 입력하세요 >>> '))      # -1000 입력해도 예외 발생되지 않음
# print(f'당신의 나이는 {age}살 입니다.')
#
# try :
#     age = int(input('나이를 입력하세요 >>> '))
#     if age < 0 or age > 200:
#         raise Exception('강제로 발생시킨 예외입니다.') # Exception 생성자
# except Exception as e:
#     print('발생한 예외 메시지는 다음과 같습니다.')
#     print(e)
'''
이상은 특정 예외가 아니라 172번으로 넘어가기만 하면 바로 예외가 발생해버린다.
즉 age에 가능한 정수값을 넣더라도 예외가 발생하기 때문에 단독으로 처리가 안되는 거다.
그렇다면 이 부분에서는 조건문을 이용해서 특정 조건일 때만 예외로 넘기는 추가 코드가 필요하다고 할 수 있겠다.
...
'''
# class NegativeAgeError(Exception):     # Exceipton 클래스를 상속받았다는 의미
#     """사용자 정의 예외 클래스 : 나이가 음수일 때 발생 """
#
# class TooManyAgeError(Exception):
#     pass
# 예외를 발생시키기만 하면 되기 때문에 뭐 굳이 코드를 쓸 필요는 없다.
# Exception 클래스로 상속 받았으면 슈퍼 클래스의 속성/method를 사용할 수 있다.
#
# try:
#     age = int(input('나이를 입력하세요 >>> '))
#     if age < 0:
...
사용자의 점수를 0 이상 100 이하로 입력 받아서 점수가 80점 이상이면 합격, 아니면 불합격을 출력하는 프로그램 작성,
0 이상 100 이하의 유효한 값이 아니라면 예외를 발생시키고
'점수는 0이상 100이하입니다.'라는 예외 메시지를 출력하시오 -> 사용자 정의 예외 클래스를 정의
ScoreOutOfRangeError 클래스를 정의해서 사용

입력 받는데 예를 들어 백점이라고 입력하면 '점수는 숫자로 입력해야합니다'라는 메시지를 출력
실수로 입력하면 '정수로 입력해야 합니다.'라는 메시지 출력

예상할 수 없는 예외가 발생시 Exception을 적용하여 디폴트 에러 메시지를 출력하세요.

프로그램이 종료되었다는 메시지 맨 마지막 작성

0점 미만에 대한 사용자 정의 클래스 추가
NegativeScoreRange -> 점수는 0이상 100이하
'''
...
'''
사용자로부터 숫자를 입력 받아 해당 숫자로 100을 나누는 값을 출력하는 프로그램 작성
만약 사용자가 0을 입력하거나 숫자가 아닌 값을 입력하면 적절한 예외 메시지를 출력하시오.

지시 사항
1. 사용자로부터 숫자를 입력 받는다.
2. 입력 받은 숫자로 100을 나누어 결과를 출력한다.
3. 입력 값이 0일 경우 적절한 예외를 처리하여 '0으로 나눌 수 없습니다'라는 몌시지 출력한다.
4. 입력 값이 숫자가 아닌 경우 적절한 예외를 처리하여 '숫자만 입력할 수 있습니다.'라는 메시지를 출력한다.
5. 예외가 발생하지 않는 경우 '정상적으로 처리되었습니다'라는 메시지 출력, 결과 출력
6. 프로그램 종료 메시지
'''
# try:
...
'''
사용자로부터 리스트의 인덱스를 입력 받아 해당 인덱스의 값을 출력하는 프로그램

지시 사항
1. 미리 정의된 리스트가 있다.
2. 사용자로부터 리스트의 인덱스를 입력 받는다.
3. 입력받은 인덱스를 사용하여 리스트의 값을 출력한다.
4. 인덱스가 리스트의 범위를 벗어나면 적절한 몌시지를 출력한다.
5. 사람을 의심하고 예상되는 예외를 적용한다.
6. 예외가 발생하지 않는 경우 "정상적으로 처리되었습니다"라는 메시지와 함께 해당 인덱스의 값 출력한다.
7. 프로그램 종료 메시지 출력
8. 마이너스인덱스는 적용시키지 않는다. -> 사용자 정의 예외 클래스 적용
    -> NegaviceNumIndexError
'''
# class NegaviceNumIndexError(Exception):
#     pass
...
#
# except NegaviceNumIndexError as e:
#     print(e)
# except IndexError as e:
...
4. 잘못된 속성명을 입력하면 '존재하지 않는 속성입니다.'라는 메시지를 출력
5. 예외가 발생하지 않은 경우 '정상적으롤 처리되었습니다.'라는 메시지와 속성값 출력
6. 프로그램 종료 메시지
'''
# class Person:
#     # 클래스 변수 선언
...
# print(vars(person1))    # vars(객체명) " 객체의 속성명 - 값을 dictionary로 만들어준다. JSON 느낌
# print(getattr(person1, 'age'))            # 결과값 : 21
# # getattr()의 두 번째 argument는 인스턴스 변수명을 받는다. -> 그 데이터를  str으로 받는다.
# print((getattr(person1,'name')))         # 결과값 : 김일

# attr_name = input('출력할 속성명을 입력하세요 >>> ')
# print(person1.school)
# getattr(person1, 'school')    # 클래스 변수인 school이 전역이 아니므로 main 단계에서 선언되지 않은 것으로 간주된다. -> 그래서 NameError가 발생했지 속성명과는 관계가 없다.
