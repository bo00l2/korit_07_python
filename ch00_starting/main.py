...
'''
python은 좀 까탈스러워서 Java나 JS할 때처럼 맨 처음이 str이면 뒤의 int/float을 알아서 바꿔주는 짓을 안한다.
...
근데 귀찮다.
그래서 f-string 개념이 도입됐다.
'''
print(f'안녕하세요 제 이름은 {name}이고, 나이는 {age}살입니다.')
'''
기억한다면 JS에서 비슷한 개념을 배웠다는 것을 알 거다.
console.log(`안녕하세요, 제 이름은 ${name}이고, 나이는 ${age}살입니다.`); 의 python판이라고 생각하면 됨.
