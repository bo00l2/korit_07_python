...
# class Korean:
#     country = '한국'
#     # 인스턴스 변수의 경우는 생성자 내부에 있었다(__init__ 내부).
#     # 클래스 변수는 이상처럼 클래스 내부에 선언하고 초기화하면 된다.
#
...
'''
객체명.클래스변수 를 통해서 클래스 변수에 접근이 가능하긴 하지만, 클래스 내부의 코드를 들여다보기 전까지는 country라는 변수가 인스턴스 변수인지 클래스 변수인지 알 방법이 없다는 문제가 있다.
...
3. 정적 메서드(static method)
    ...
    - 클래스 메서드와의 공통점 # 1
        elf.속성명을 사용하여 인스턴스 변수의 값을 참조하는데 정적 메서드는 아예 첫 번째 매개변수가 고정이 아니다. 
    - 클래스 메서드와의 공통점 # 2
        인스턴스를 생성하지 않아도 사용할 수 있음
        
    특징 :
        1) 인스턴스 / 클래스로 호출 가능 -> 클래스 메서드와의 공통점
        2) 생성된 인스턴스가 없어도 호출 가능 _> 클래스 메서드와의 공통점
        
        3) 클래스 메서드와의 차이점 # 1 -> @staticmethod 데코레이터를 표기하고 작성 
        4) 클래스 메서드와의 차이즘 # 2 -> 반드시 작성해야 할 매개변수(self / cls)가 없음 
        
이상을 토대로, 정적 메서드는 self / cls를 둘 다 사용하지 않기 때문에 인스턴스 / 클래스 변수를 모두 사용하지 않는 메서드를 정의하는 경우에 적합하다.
정적 메서드 자체는 클래스에 소속되어있지만 인스턴스에는 영향을 주지도 않고 받지도 않는다.
...
#     count = 0
#
#     def __init__(self): # 생성자 호출 및 인스턴스 변수들 정의할 용도. 그럼 생성자도 인스턴스 변수라고 할 수 있겠다(self가 있으니까)
#         Bag.count += 1      # 생성자가 호출될 때마다(=객체가 생성될 때마다) 클래스 변수인 count가 1씩 증가함. cls.count가 아니라 클래스명.count라는 것에 주목해야 한다.
...
#         print('가방이 팔렸습니다.')
#         cls.count -= 1
#         # 얘는 클래스 메서드가 클래스변수에 접근한 것이기 때문에 Bag.count가 아니라 cls.count로 작성되었다.
#
#     @classmethod
#     def remain_bag(cls):
#         return cls.count
...
Shop.sales('김밥', 2)        # 김밥을(를) 1 개 판매
Shop.sales('튀김', 5)        # 튀김을(를) 1 개 판매

3. 모든 매출을 작성한 뒤 다음과 같은 방식으로 전체 매출액을 확읺나다.
...
class Shop:
    ...
    def sales(cls, food, sel):
        print(f'{food}을(를) {sel} 개 판매')
        for dic in cls.menu_list:
            for key in dic:
                if key == food:
